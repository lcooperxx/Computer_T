...
void CglMath::CalculateNormal(CVector3 *vert, CVector3 *norm, int numRow, int numCol)
{
	//初始化法向量为0
	int nIndex = 0, i, j;
	for (i = 0; i<numRow; i++)
	...
	for (i = 0; i<numRow - 1; i++)
	{
		for (j = 0; j<numCol; j++)
		{
			...
			norm[index[0]] = norm[index[0]] + facenormal;
			norm[index[1]] = norm[index[1]] + facenormal;
			norm[index[2]] = norm[index[2]] + facenormal;
			facenormal = (vert[index[2]] - vert[index[3]]).crossMul(vert[index[0]] - vert[index[3]]);
			norm[index[2]] = norm[index[2]] + facenormal;
			norm[index[3]] = norm[index[3]] + facenormal;
			norm[index[0]] = norm[index[0]] + facenormal;
		}
	}
	//3、所有法向量单位化
...
void CglMath::Dir2HPR(CVector3 dir, CVector3 up, CVector3 &hpr)
{
	dir.Normalize();
	up.Normalize();
	if (dir.m_dY == 1 || dir.m_dY == -1)//万向锁
	{
		dir.m_dY += 0.001;
		dir.m_dX = 0.001;
		dir.m_dZ = 0.001;
	...
	CVector3 dir_XoZ(dir.m_dX, 0, dir.m_dZ,0);
	dir_XoZ.Normalize();
	double temp = dir_XoZ.dotMul(dir);
	temp = max(min(temp, 1), -1);
	hpr.m_dY = double(RadToAng(acos(temp)));//p值，绕x旋转
	if (dir.m_dY<0)
		hpr.m_dY = -hpr.m_dY;

	hpr.m_dX = double(RadToAng(acos(-dir_XoZ.m_dZ)));//h值，绕y旋转
	...
	//这里q=(0,1,0);d=0;n=dir.
	CVector3 dir_upStd, dir_up;
	CVector3 Ydir(0, 1, 0,0);
	...
	temp = max(min(temp, 1), -1);
	hpr.m_dZ = double(RadToAng(acos(temp)));
	//判断正负
	CVector3 dir_leftStd = dir_upStd.crossMul(dir);
	temp = dir_leftStd.dotMul(dir_up);
	if (temp<0) hpr.m_dZ = -hpr.m_dZ;
}
