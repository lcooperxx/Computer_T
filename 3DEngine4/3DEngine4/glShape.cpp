...
void CglShape::DrawBall(float radius, int lon, int lat)//使用经度分割lon,维度分割lat,半径radius绘制一个球。
{
	if (!m_nVert)
	{
		...
		lon += 1;
		CVector3 facenormal;
		m_vertArray = new CVector3[lon*(lat + 1)];
		m_normalArray = new CVector3[lon*(lat + 1)];
		m_texArray = new CVector3[lon*(lat + 1)];
		...
		for (lonCur = 0, i = 0; i<lon; lonCur += lonStep, i++)	//经度
		{
			for (latCur = -PAI / 2, j = 0; j <= lat; latCur += latStep, j++)	//维度
			{
				...
				m_normalArray[nVert].Set(0, 0, 0,0);
		...
		for (i = 0; i<lon; i++)	//经度
		{
			for (j = 0; j<lat; j++)	//维度
			{
				...
				m_normalArray[index[0]] = m_normalArray[index[0]] + facenormal;
				m_normalArray[index[1]] = m_normalArray[index[1]] + facenormal;
				m_normalArray[index[2]] = m_normalArray[index[2]] + facenormal;
				facenormal = (m_vertArray[index[2]] - m_vertArray[index[3]]).crossMul(m_vertArray[index[0]] - m_vertArray[index[3]]);
				m_normalArray[index[2]] = m_normalArray[index[2]] + facenormal;
				m_normalArray[index[3]] = m_normalArray[index[3]] + facenormal;
				m_normalArray[index[0]] = m_normalArray[index[0]] + facenormal;
			}
		}
		//3、所有法向量单位化
