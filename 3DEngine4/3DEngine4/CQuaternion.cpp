...
void CQuaternion::Set(double x, double y, double z, double w)//设置对应的值
{
	this->x = x;
	this->y = y;
	this->z = z;
...
void CQuaternion::SetAngle(double angle, CVector3 axis)//四元数设置
{
	double halfangle;
...
CQuaternion& CQuaternion :: operator=(CQuaternion& p) //重载赋值
{
	(*this).x = p.x;
	(*this).y = p.y;
	(*this).z = p.z;
...
CQuaternion CQuaternion :: operator+(CQuaternion& p) //重载‘+’
{
	(*this).w += p.w;
	(*this).x += p.x;
	(*this).y += p.y;
...
CQuaternion CQuaternion :: operator*(double data) //重载数乘
{
	(*this).w *= data;
	(*this).x *= data;
	(*this).y *= data;
...
CQuaternion CQuaternion :: operator*(CQuaternion&p) //四元数乘法
{
	CQuaternion cq1;
...
CQuaternion CQuaternion::Div(CQuaternion&b) //求差 当前为a,求c=a-b
{
	(*this) = (*this).Inverse();
	(*this) = (*this) * b;
	return (*this);
}
double CQuaternion::dotMul(CQuaternion&p) //点乘
{
	return (*this).x * p.x + (*this).y * p.y + (*this).z * p.z + (*this).w * p.w;
}
void CQuaternion::GetAngle(double& angle, CVector3& axis) //求旋转轴和角度
{
	(*this).Normalize();
...
CQuaternion CQuaternion::Slerp(CQuaternion& Vend, double t) //插值。从当前四元数插值到Vend四元数,t是参数[0,1]
{
...
void CQuaternion::Slerp(CQuaternion& Vend, int n, double *t, CQuaternion *Result)//插值。一次插值出n个数据。插值参数保存在数组t中，结果返回到数组Result中
{
	for (int i = 0; i < n; i++)
	{
