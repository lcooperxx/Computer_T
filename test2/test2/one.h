...
void Scatter_Line()
{
	...
	for (int i = 0; i < point.size() - 1; i++){
		...
		glVertex3f(z[0][point[i]], z[1][point[i]], z[2][point[i]]);//直线起点
		glVertex3f(z[0][point[i + 1]], z[1][point[i + 1]], z[2][point[i + 1]]);//直线终点
...
void Scatter_triangle()
{
	...
	for (int i = 1; i < point.size() - 1; i++){
		...
		glVertex3f(z[0][point[i - 1]], z[1][point[i - 1]], z[2][point[i - 1]]);//三角形第一个点
		glVertex3f(z[0][point[i]], z[1][point[i]], z[2][point[i]]);//三角形第二个点
		glVertex3f(z[0][point[i + 1]], z[1][point[i + 1]], z[2][point[i + 1]]);//三角形第三个点
		//glVertex3f(z[0][i], z[1][i], z[2][i]);
		//glVertex3f(z[0][i+1], z[1][i+1], z[2][i+1]);
		glEnd();
	}
